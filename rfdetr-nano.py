...
# Configure device - Use GPU if available for faster inference
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"Using device: {device}")

# 2. Load RF-DETR Nano pretrained model
processor = AutoImageProcessor.from_pretrained("roboflow/rf-detr-nano")
model = RfDetrForObjectDetection.from_pretrained("roboflow/rf-detr-nano")
model.to(device)
model.eval()
